...
void BipartiteGraph::addEdge( size_t n1, size_t n2, bool check ) {
    ...
    if( check ) {
        ...
        daiforeach( const Neighbor &nb2, nb1(n1) )
            if( nb2 == n2 ) {
                exists = true;
                break;
            }
...
std::vector<size_t> BipartiteGraph::delta1( size_t n1, bool include ) const {
    ...
    daiforeach( const Neighbor &n2, nb1(n1) )
        daiforeach( const Neighbor &m1, nb2(n2) )
            if( include || (m1 != n1) )
                result.push_back( m1 );
    // remove duplicates
...
std::vector<size_t> BipartiteGraph::delta2( size_t n2, bool include ) const {
    ...
    std::vector<size_t> result;
    daiforeach( const Neighbor &n1, nb2(n2) )
        daiforeach( const Neighbor &m2, nb1(n1) )
            if( include || (m2 != n2) )
                result.push_back( m2 );
    // remove duplicates
    std::vector<size_t>::iterator it = std::unique( result.begin(), result.end() );
    ...
    if( nrNodes1() == 0 || nrNodes2() == 0 )
        ...
        do {
            ...
            if( levels.size() == 0 ) {
                ...
                // build newLevel.ind1
                daiforeach( size_t n2, prevLevel.ind2 ) { // for all n2 in the previous level
                    daiforeach( const Neighbor &n1, nb2(n2) ) { // for all neighbors n1 of n2
                ...
                // build newLevel.ind2
                daiforeach( size_t n1, newLevel.ind1 ) { // for all n1 in this level
                    daiforeach( const Neighbor &n2, nb1(n1) ) { // for all neighbors n2 of n1
