...
void GraphAL::addEdge( size_t n1, size_t n2, bool check ) {
    ...
    if( check ) {
        ...
        daiforeach( const Neighbor &n, nb(n1) )
            if( n == n2 ) {
                exists = true;
                break;
            }
    ...
    if( nrNodes() == 0 ) {
        ...
        do {
            ...
            for( size_t n1 = 0; n1 < nrNodes(); n1++ )
                if( !incomponent[n1] ) {
                    daiforeach( const Neighbor &n2, nb(n1) ) {
                        if( incomponent[n2] ) {
    ...
    if( nrNodes() == 0 )
        ...
        do {
            ...
            if( levels.size() == 0 ) {
                ...
                // build newLevel
                daiforeach( size_t n2, prevLevel ) { // for all n2 in the previous level
                    daiforeach( const Neighbor &n1, nb(n2) ) { // for all neighbors n1 of n2
...
void GraphAL::checkConsistency() const {
    ...
    for( size_t n2 = 0; n2 < N; n2++ ) {
        size_t iter = 0;
        daiforeach( const Neighbor &n1, nb(n2) ) {
