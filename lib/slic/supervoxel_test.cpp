          ...
          if(width == -1 && height == -1)
            {
              ...
              printf("Image size=(%d,%d) widthStep=%d\n",width,height,img_slice->widthStep);
            }
        }
    }
      ...
      if(img_slice->width != width || img_slice->height != height)
      ...
      else
        {
          ...
          if(img_slice->nChannels != bytes_per_pixel)
            {
              // FIXME
              printf("Error: img_slice->nChannels != bytes_per_pixel %d %d\n", img_slice->nChannels, bytes_per_pixel);
              exit(-1);
            }
          ...
          if(img->widthStep != img->width)
            {
              ...
              memset((char*)data[d],0,n*sizeof(int));
              uchar* raw_data = (uchar*) data[d];
              int i = 0;
              for(int y = 0; y < img->height ; y++)
                for(int x = 0; x < img->width ; x++)
                  {
                    for(int z = 0; z < img_slice->nChannels ; z++)
                      {
                        raw_data[i] = ((uchar*)(img->imageData + img->widthStep*y))[x*img->nChannels+z];
                        i++;
                      }
                    i++; // skip one channel
                  }
...
int main(int argc, char* argv[])
{
  ...
  printf("Exporting supervoxel cube containing %d labels to %s\n",numlabels, output_name);

  unsigned int n = width*height;
  ofstream ofs(output_name, ios::binary);
  ...
  printf("------------\n");
  printf("Test : reloading supervoxel cube containg %d voxels\n",n);
  int* labels = new int[n];
  ifstream ifs(output_name, ios::binary);
  for(int d=0;d<depth;d++)
    {
